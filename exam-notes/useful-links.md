...
| [dev.to/aws-builders/…](https://dev.to/aws-builders/the-claude-certified-architect-exam-5-domains-6-scenarios-and-everything-you-need-to-know-4le3) | Long-form overview article (5 domains, 6 scenarios, exam structure) | 🔵 Third-party | Free, no registration | Early–mid phase: orientation + resource map |
| [claude-guides.com](https://claude-guides.com/) | Claude Code reference hub (command cheatsheets, shortcuts, how-to guides) | 🔵 Third-party | Free, no registration | Any phase: D3 (Claude Code config) lookup, not exam-specific drilling |
| [github.com/paullarionov/claude-certified-architect](https://github.com/paullarionov/claude-certified-architect) | Study guides + practical exercises (PDFs, HTML tests) | 🔵 Third-party | Free/public, no login | Early–mid phase: guided reading + hands-on exercises across domains |
...
### 🔵 dev.to/aws-builders/… — long-form exam overview
**What it covers:** a free third-party article by Brayan Arrieta, published via the AWS Community Builders program (not affiliated with Anthropic). It's a structural walkthrough of the exam rather than a question bank: the same 5 domains with their weights (D1 27% / D2 18% / D3 20% / D4 20% / D5 15%), the 6 scenarios (Customer Support, Code Generation, Multi-Agent Research, Developer Productivity, CI/CD, Structured Data Extraction), the exam frame, and a map of where to find study resources. It references the official `anthropic.skilljar.com` and exam guide but keeps editorial independence.
**Access:** fully free to read, no login or payment. AWS-Builders / community piece — **not affiliated with Anthropic**.
**When it's useful:** early–mid phase — **orientation and a resource map**: get the lay of the land (domains, weights, scenarios) and identify gaps before you commit to drilling. It's an overview, not deep technical practice, so it doesn't replace `/teach`/`/quiz`/`/mock`; cross-check any concrete claim against authoritative sources before recording it.

### 🔵 claude-guides.com — Claude Code reference hub
**What it covers:** a free third-party reference site for **Claude Code itself** (author Paul Larionov; not affiliated with Anthropic) — command cheatsheets, keyboard shortcuts, and procedural how-to guides. It's a lookup tool, not an exam study platform: there are **no exam domain breakdowns, scenarios, practice questions, mocks, or flashcards** here (it links a "Claude Certified Architect" GitHub repo but that's separate). Because it documents the product, it overlaps most with **D3 (Claude Code Config & Workflows)**.
**Access:** fully free, no login or payment. Community/personal site — **not affiliated with Anthropic**, so treat command/setting details as needing a cross-check against `code.claude.com`.
**When it's useful:** any phase as a **quick reference** for Claude Code commands, settings, and config — handy when a D3 question hinges on what a command or setting actually does. Not a source for first learning the exam concepts, and not retrieval practice; verify specifics against `code.claude.com` before recording them.

### 🔵 github.com/paullarionov/claude-certified-architect — study guides + exercises
**What it covers:** a free, public GitHub repo (same author as `claude-guides.com`, Paul Larionov; not affiliated with Anthropic) with written study guides plus **hands-on practical exercises** covering tool design, MCP integration, structured output, and context-management/reliability patterns, with HTML practice tests. Guides ship in several languages with PDF versions. It links out to free Anthropic Academy courses. There's **no full scaled mock or large question bank** — it's guided reading + exercises, not a simulation. License is **not stated** in the repo, so treat reuse cautiously.
**Access:** fully free and public on GitHub, no login. Community/personal project — **not affiliated with Anthropic**.
**When it's useful:** early–mid phase — **guided learning and hands-on practice** across the domains, a complement to `exam-prep/` for working through scenarios. Weak as a late-stage readiness check (no full mock); cross-check concrete claims against authoritative sources before recording them.
